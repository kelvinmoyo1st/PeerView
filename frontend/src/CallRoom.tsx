...
function ReviewPanel({ sessionId, evaluations, report, onReport, onClose }: { sessionId: string; evaluations: Evaluation[]; report: string | null; onReport: (report: string) => void; onClose: () => void }) {
  ...
  return <main className="review-page"><header className="call-header"><a className="brand" href="/"><span className="brand-mark" aria-hidden="true">PV</span><span>PeerView</span></a><button className="end-button" type="button" onClick={onClose}>Dashboard</button></header><section className="review-content"><p className="eyebrow">Interviewer review</p><h1>Make the final call.</h1><p className="review-intro">The AI suggestions are a second opinion. Your scores below are the authoritative result.</p>{report ? <div className="report-box"><span className="number">Final report</span><p>{report}</p></div> : <form onSubmit={submit} className="review-form">{evaluations.map((evaluation, index) => <article className="review-item" key={evaluation.questionId}><h2>{evaluation.question}</h2><p className="ai-suggestion">AI suggestion: {evaluation.aiScore}/5. {evaluation.aiFeedback}</p><label>Your score (1-5)<input type="number" min="1" max="5" value={items[index].score} onChange={(event) => setItems((current) => current.map((item, itemIndex) => itemIndex === index ? { ...item, score: Number(event.target.value) } : item))} required /></label><label>Your feedback<textarea value={items[index].feedback} onChange={(event) => setItems((current) => current.map((item, itemIndex) => itemIndex === index ? { ...item, feedback: event.target.value } : item))} /></label></article>)}<label>Overall notes<textarea value={notes} onChange={(event) => setNotes(event.target.value)} /></label>{error && <p className="form-error" role="alert">{error}</p>}<button className="primary-action" type="submit">Submit final review <span aria-hidden="true">&#8594;</span></button></form>}</section></main>
}